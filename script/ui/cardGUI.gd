...
func _ready():
	...
	mouse_exited.connect(_stop_hover)
...
func _process(_delta):
	...
	if dragging and card_logic.target_drag:		
		arc.visible = true
...
func _stop_hover():
	...
	# Tweens position to normal when mouse stops hovering
	hover_tween = create_tween()
...
func end_drag():
	if not can_drag or not dragging:
		return
	if hover_tween:
		hover_tween.kill()
